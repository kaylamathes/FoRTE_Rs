...
This R Mardown document summarizes data analysis and figures for the manuscript in prep: 
"Sustained decline in soil respiration with increasing disturbance severity three years following experimental disturbance"

Authors: Kayla C. Mathes*, Stephannie Pennington, Carly Rodrigeuz,  Ben Bond-Lamberty, Christoph S. Vogel, Christopher M. Gough

For open assess data from the FoRTE project go to: <https://github.com/kaylamathes/fortedata>
...
```{r echo=FALSE, message=FALSE, fig.width=30, fig.height=20}
...
```
**Figure 1**: Mean (± 1 S.E.) measurement period soil respiration (Rs; A, B), soil temperature (Ts; C, D), and volumetric water content (VWC; E, F) by disturbance severity (left panels) and treatment type (right panels), 2018-2021.  Red vertical dashed line delineates pre- and post-disturbance measurement periods.  


####Model assumption Testing for Rs split-split model 
...
###Running Rs split-split ANOVA models with and without covariates (Chosen model was based on lowest AIC score)
```{r echo = FALSE, message = FALSE}
...
```
**Figure 2**: Boxplots displaying median, interquartile range (middle 50% of range) and minimum and maximum soil respiration (Rs) values by disturbance severity for pre-disturbance (2018, gray shading) and post-disturbance (2019-2021) years. Different letters indicate significant within-year differences among disturbance severities (alpha = 0.05, F = 4.42, p < 0.001). 
...
ggplot(all_years_summary_treatment,aes(x = Treatment, y = ave_efflux, fill = Treatment)) +
  ...
  geom_rect(data=data.frame(year='2018'), inherit.aes=FALSE,
            ...
            fill = 'grey20', alpha = 0.2)
```
**Figure 3**: Boxplots displaying median, interquartile range (middle 50% of range) and minimum and maximum soil respiration (Rs) values by top-down and bottom-up disturbance types for pre-disturbance (2018, gray shading) and post-disturbance (2019-2021) years. Within-year comparisons yielded no significant differences (alpha = 0.05, F = 0.946, p = 0.42). 

####Model assumption Testing for Rh split-split model 
```{r echo = FALSE, message = FALSE}
####Model assumption Testing for Rh split-split model 
#Rh Model 
####Testing Assumptions 
...
###Running Rh split-split ANOVA models with and without covariates (Chosen model was based on lowest AIC score)
```{r echo = FALSE, message = FALSE}
...
Rh_T <- ggplot(all_years_Rh_treatment_ONLY, aes(x = Treatment, y = ave_soilCO2Efflux_umolg, fill = Treatment)) +
  theme_classic()+
  scale_fill_manual(values=c("#A6611A", "#018571"))+
  ...
  annotate("text", x = 0.6, y = 0.0063, label = "B", size = 9)


##Plot Rh Severity all years
Rh_S <- ggplot(all_years_Rh_severity_ONLY, aes(x = Severity, y = ave_soilCO2Efflux_umolg, fill = Severity)) +
  theme_classic()+
  scale_fill_manual(values=c("#000000", "#009E73", "#0072B2", "#D55E00"))+
  geom_boxplot()+
  theme(axis.text.x= element_text(size = 25), axis.text.y= element_text(size=25), axis.title.x = element_text(size = 30), axis.title.y  = element_text(size=30), legend.title = element_blank(),  strip.text.x =element_text(size = 25), legend.text = element_blank(), legend.position = "none",panel.background = element_rect(fill = NA, color = "black")) +
...
Rh_T_grob <- ggplotGrob(Rh_T)
Rh_S_grob <- ggplotGrob(Rh_S)


layout <- rbind(c(1,2))


Rh <- grid.arrange(Rh_S_grob, Rh_T_grob, layout_matrix=layout)
```
**Figure 4**: Boxplots displaying median, interquartile range (middle 50% of range) and minimum and maximum soil heterotrophic respiration estimates (Rs) of incubated root-free soils (10 cm depth) averaged across all post-disturbance years (2019-2021) by: A) disturbance severity (alpha = 0.05, F= 4.369, p = 0.042) and B) top-down and bottom-up disturbance type (alpha = 0.05, F = 6.837, p = 0.024). Different letters indicate significant differences among disturbance severities or disturbance type. 
...
ggplot(all_years_Q10, aes(x = ave_soilTemp, y = ave_soilCO2Efflux, group = Severity)) +
  ...
  scale_color_manual(values=c("#000000", "#009E73", "#0072B2", "#D55E00")) +
    geom_smooth(method = "nls",
                ...
                se = FALSE,
                aes(color = Severity)) +
        theme(axis.text.x = element_text(size = 20), axis.text.y= element_text(size=25), axis.title.x = element_text(size = 20),   axis.title.y  = element_text(size=25), legend.title = element_text(size = 20),  strip.text.x =element_text(size = 25), legend.text = element_text(size = 15), panel.background = element_rect(fill = NA, color = "black")) +
 labs(x = "Soil Temperature", y=expression(paste(" ",R[s]," (",mu*molCO[2],"  ",m^-2,"  ",sec^-1,")")))
 
ggsave(path = "Manuscript_figures", filename = "Q10_plot_severity.png", height = 10, width = 10, units = "in")


##All Data points by day of measurement faceted by treatment
ggplot(all_years_Q10, aes(x = ave_soilTemp, y = ave_soilCO2Efflux, group = Treatment)) +
...
ggsave(path = "Manuscript_figures", filename = "Q10_plot_treatment.png", height = 10, width = 10, units = "in")

```
**Figure 5**: Post-disturbance (2019-2021) Soil respiration temperature response curves (Q10) by Severity and Treatment. 
...
##Severity Q10
Q10_1 <- ggplot(param_model_Q10_b, aes(x = Severity, y = Q10, fill = Severity)) +
  theme_classic()+
  scale_fill_manual(values=c("#000000", "#009E73", "#0072B2", "#D55E00"))+
  geom_boxplot()+
  ...
  annotate("text", x = 0.6, y = 3, label = "A", size = 12)

##Severity Intercept
Q10_3 <- ggplot(param_model_Q10_a, aes(x = Severity, y = intercept, fill = Severity)) +
  theme_classic()+
  scale_fill_manual(values=c("#000000", "#009E73", "#0072B2", "#D55E00"))+
  geom_boxplot()+
  ...
  scale_y_continuous(sec.axis = sec_axis(~ .,labels = NULL))+
  labs(x = "Severity (% Gross Defoliation)", y = "Intercept")+
  annotate("text", x = 0.6, y = 2.43, label = "C", size = 12)

##Treatment Q10
##Boxplots of Q10 Values and intercept values 
Q10_2 <- ggplot(param_model_Q10_b, aes(x = Treatment, y = Q10, fill = Treatment)) +
  theme_classic()+
  scale_fill_manual(values=c("#A6611A", "#018571"))+
  geom_boxplot(width = 0.5)+
  theme(axis.text.x= element_blank(), axis.text.y= element_text(size=30), axis.title.x = element_blank(), axis.title.y  = element_text(size=35), legend.title = element_blank(),  strip.text.x =element_text(size = 25), legend.text = element_blank(), legend.position = "none",panel.background = element_rect(fill = NA, color = "black")) +
  ...
  annotate("text", x = 0.5, y = 3, label = "B", size = 12)

##Treatment Intercept
Q10_4 <- ggplot(param_model_Q10_a, aes(x = Treatment, y = intercept, fill = Treatment)) +
  theme_classic()+
  scale_fill_manual(values=c("#A6611A", "#018571"))+
  geom_boxplot(width = 0.5)+
  theme(axis.text.x= element_text(size = 30), axis.text.y= element_text(size=30), axis.title.x = element_text(size = 35), axis.title.y  = element_text(size=35), legend.title = element_blank(),  strip.text.x =element_text(size = 25), legend.text = element_blank(), legend.position = "none",panel.background = element_rect(fill = NA, color = "black")) +
  scale_y_continuous(sec.axis = sec_axis(~ .,labels = NULL), position = "right")+
  labs(x = "Treatment", y = "Intercept")+
  annotate("text", x = 0.5, y = 2.43, label = "D", size = 12)
...
layout_Q10 <- rbind(c(1,2),
                    c(3,4))
g_Q10 <- grid.arrange(Q10_1_grob, Q10_2_grob, Q10_3_grob, Q10_4_grob,layout_matrix=layout_Q10)

ggsave(path = "Manuscript_figures", filename = "Q10_intercept.png", height = 15, width = 20, units = "in",g_Q10 )

```
**Figure 6**: Mean Q10 and Intercept values by severity and treatment. 

###Split-plot model for Q10 and Intercept means by replicate across Severity and Treatment 
```{r echo = FALSE, message = FALSE}
...
```
**Figure 7**: A) Average Rs and B) Average Rh resistance (rt) as a function of disturbance severity (% gross defoliation), 2018-2021. Gray line indicates significant linear decline in Rs resistance with increasing disturbance severity between 2019-2021 (p-value: <0.001, Adjusted R2 = 0.93). No significant relationship between Rh and disturbance severity. 
